...
function ImageView(props: any) {
    ...
    const { align, inline, indent } = props?.node?.attrs || {}
    ...
    const imgAttrs = useMemo(() => {
        ...
        const width = isNumber(w) ? `${w}` + (w == 100 ? '%' : 'px') : w
        const height = isNumber(h) ? `${h}` + (h == 100 ? '%' : 'px') : h
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
        <NodeViewWrapper
            as={inline ? 'span' : 'div'}
            className={`image-view image image-align-${align || 'center'}`}
            ...
            style={{
                ...
                margin: inlineFloat
                    ? align === 'left'
                        ...
                        : '1em 0 1em 1em'
                    : undefined,
                marginLeft: indent ? `${indent * 30}px` : undefined,
                display: inline ? 'inline-block' : 'block',
                textAlign: indent ? 'left' : (inlineFloat ? undefined : align),
