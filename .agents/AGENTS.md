...
- **Quy chuẩn Hình ảnh (Image Upload)**: Mọi trường dữ liệu hình ảnh (Ảnh bìa, Ảnh đại diện, Banner, Logo, Custom Icon) bắt buộc phải sử dụng component dùng chung `<ImageUploader />` (`client/src/components/ImageUploader.tsx`), hỗ trợ tải tệp từ máy tính (kéo thả Drag & Drop / chọn file) hoặc nhập link URL linh hoạt.
- **Quy chuẩn Thông báo (Toast Notification)**: Mọi phản hồi thao tác người dùng bắt buộc phải sử dụng `<Toast />` component (`client/src/components/Toast.tsx`) và phân biệt rõ ràng 2 trạng thái: `success` (Thành công - Viền/Badge xanh lá) và `error` (Thất bại/Lỗi - Viền/Badge đỏ).
